...
ec.analytics = function(c) {
    if (c.bd) {
        var a = "https://hm.baidu.com/h.js?82d2186024cf7459f80be3ff94bea77f";
        ec.util.addScriptTag(a)
    }
    if (c.dmp) {
        var b = "https://nebula-collector.huawei.com/api/2.0/vmallcn-min.js";
        ec.util.addScriptTag(b)
    }
    if (c.bi || c.dap) {
        var b = "https://res.vmallres.com/bi/hianalytics_v15.js";
        ec.util.addScriptTag(b)
    }
    if (c.bi) {
...
ec.fireAgainRequestForVmall = function() {
    ...
    ec.account = {
        ...
        setLoginInfo: function() {
            ...
            e.expires = 1;
            if (c.cid) {
                b.set("cps_id", decodeURIComponent(c.cid), e)
            }
            if (c.wi) {
                b.set("cps_wi", decodeURIComponent(c.wi), e)
            }
            if (c.tag && c.tag == 1) {
                if (ec.isRequestFromVmall) {
                    ec.setLoginUrlForVmall()
    ...
    ec.ajax({
        ...
        success: function(k) {
            if (k && k.success) {
                ...
                if (k && k.code) {
                    if (k.code == 4 || k.code == 6 || k.code == 8 || k.code == 9) {
...
ec.submit = function(c) {
    ...
    ec.creatOrderUrl = "https://ord01.vmall.com/order/pwm86t/createOrder.do";
    if (ec.retryFun) {
        clearTimeout(ec.retryFun)
    }
    if (ec.oneMin) {
...
ec.getStatus = function(a) {
    ...
    case 2:
        ...
        break;
    case 3:
        $("boxHeader").innerHTML = '<img src="https://res.vmallres.com/cn/activity/rushbuy/20190901/cn/web/images/ic_cry.jpg">';
...
ec.showMessageBox = function(c, a, b, f) {
    ...
    if (ec.isRequestFromVmall && c != 1 && c != 2 && c != 3 && c != 9 && c != 10 && c != 11 && c != 12 && c != 13) {
        return
    }
    ...
    if (c == 1 || c == 10 || c == 13) {
        if (c == 1 || c == 13) {
            $("boxHeader").innerHTML = '<img src="https://res.vmallres.com/cn/activity/rushbuy/20190901/cn/web/images/ic_expect.jpg">'
        } else {
            $("boxHeader").innerHTML = '<img src="https://res.vmallres.com/cn/activity/rushbuy/20190901/cn/web/images/ic_smile.jpg">'
        }
    ...
    if (c == 1) {
        $("boxText").innerHTML = '<p class="honor-end-pd" style="font-size: 14px; color: #000000;">抱歉，没有抢到</p>' + honorEndTxt;
        $("boxButton").innerHTML = '<a href="' + ec.url.activity + '" title="返回活动" class="honor-box-btn" >返回活动</a>' + ec.isYxgUrl
    } else {
        if (c == 4) {
            $("boxText").innerHTML = "活动未开始，请校准本机时间到北京时间后再刷新页面";
...
ec.isAgreementRules = function() {
    var b = ec.util.cookie.get("hasSigned");
    if (ec.account.isLogin() && (b == 0 || !b)) {
        var a = {
            portal: 1,
            lang: "zh-CN",
            country: "CN",
        ...
        a = JSON.stringify(a);
        ec.ajax({
            url: ec.url.ucDomain + "/mcp/agreement/queryAgreementSignLogs",
            type: "POST",
            data: a,
            headers: true,
            xhrFields: {
            ...
            success: function(d) {
                if (d && d.success) {
                    var c = d.signInfo;
                    if (c.length > 0) {
                        if (c[0].agree && c[1].agree && !c[0].needSign && !c[1].needSign) {
                            ec.util.cookie.set("hasSigned", 1);
                            ec.hide(["agreement-rules", "masked"])
                        } else {
                            if (!c[0].agree || !c[1].agree) {
                                agreement_type = 0;
                                agreement_name = "华为商城通知";
                                ec.show(["agreement-rules", "masked"])
                            } else {
                                if (c[0].needSign || c[1].needSign) {
                                    if (c[0].needSign && c[1].needSign) {
                                        agreement_type = 3;
                                        agreement_name = "变更通知";
                                        ec.show(["agreement-rules", "masked"]);
                                        $("agreement_header").innerText = "变更通知";
                                        $("agreement_content_tip").innerHTML = '为了给您提供更好的服务，我们更新了服务协议和隐私声明。<p style="margin-bottom: .5rem;"></p>详情内容请参见最新的'
                                    } else {
                                        if (c[0].needSign && !c[1].needSign) {
                                            agreement_type = 1;
                                            agreement_name = "服务协议变更通知";
                                            ec.show(["agreement-rules", "masked"]);
                                            $("agreement_header").innerText = "服务协议变更通知";
                                            $("agreement_content_tip").innerHTML = '为了给您提供更好的服务，我们更新了服务协议。<p style="margin-bottom: .5rem;"></p>详情内容请参见最新的';
                                            ec.hide(["agreement_cloudservice", "agreement_icon"])
                                        } else {
                                            agreement_type = 2;
                                            agreement_name = "隐私声明更新";
                                            ec.show(["agreement-rules", "masked"]);
                                            $("agreement_header").innerText = "隐私声明变更通知";
                                            $("agreement_content_tip").innerHTML = '为了给您提供更好的服务，我们更新了隐私声明。<p style="margin-bottom: .5rem;"></p>详情内容请参见最新的';
                                            ec.hide(["agreement_terms", "agreement_icon"])
                                        }
                                    }
                                }
                            }
...
ec.disAgreement = function() {
    agreement_option = "disagree";
    var a = {
        type: agreement_type,
        name: agreement_name,
        option: agreement_option,
        marketing: "",
        click: "1"
    };
    _paq.push(["setTrackerUrl", "https://dap.vmall.com/dap/report"]);
    _paq.push(["trackLink", 300000701, "link", a]);
    ec.pushData();
    window.location.href = ec.url.vmall + "/account/logout"
};
ec.agreement = function() {
    var a = {
        portal: 1,
        lang: "zh-CN",
        country: "CN",
    ...
    ec.ajax({
        url: ec.url.ucDomain + "/mcp/agreement/signAgreement",
        type: "POST",
        data: a,
        headers: true,
        xhrFields: {
        ...
        success: function(c) {
            ...
            ec.hide(["agreement-rules", "masked"]);
            _paq.push(["setTrackerUrl", "https://dap.vmall.com/dap/report"]);
            _paq.push(["trackLink", 300000701, "link", b]);
            ec.pushData()
        },
        error: function() {
            ec.util.cookie.set("hasSigned", 1);
...
ec.excuteRush = function(){
	
	var stopFlag = ec.util.cookie.get("t_stopFlag") || "0";
	...
	var qUid = ec.util.cookie.get("uid");
	var qualificationId = ec.util.cookie.get("t_qualificationId") || ec.qid;
	var qualificationUrl = ec.util.cookie.get("t_qualificationUrl") || ec.url.qualification;
	var qualificationNeedQueue = ec.util.cookie.get("t_qualificationNeedQueue") || ec.needQueue;
	console.log("===准备开始排队:"+[qUid,qualificationId,qualificationUrl,qualificationNeedQueue].join(","));
	if (qUid && qualificationId) {
			...
			success: function(d) {
				...
				if (d) {
					if (d.isqueue && d.queueSign) {
						...
						ec.util.cookie.set("isqueue-" + ec.activityId + "-" + qUid, d.isqueue, f);
						ec.util.cookie.set("queueSign-" + ec.activityId + "-" + qUid, d.queueSign, f);
						if (qualificationNeedQueue && d.isqueue != 2) {
							console.log("===排队成功,执行提交")
							ec.excuteSubmit();
							return;
						}
					}
				...
				if (qualificationNeedQueue){
					ec.excuteRush();
				}else{
					console.log("===不需要排队，执行提交")
					ec.excuteSubmit();
				}
			}
		})
...
ec.excuteSubmit = function() {
	...
	console.log("===执行excuteSubmit")
	var intervalTime = ec.util.cookie.get("t_intervalTime") || 300;
	var submitCreatOrderUrl = ec.util.cookie.get("t_submitCreatOrderUrl") || "https://ord01.vmall.com/order/pwm86t/createOrder.do";
	var submitCoverFlag = ec.util.cookie.get("t_submitCreatOrderUrl") || coverFlag;
	var accountPars = ec.account.getLoginPars();
	accountPars.skuId = ec.selectedProduct.skuId;
    accountPars.skuIds = ec.selectedProduct.skuId;
    accountPars.activityId = ec.util.cookie.get("t_submitActivityId") || ec.activityId;
    ...
    accountPars.nowTime = currDate;
	
	skus = "&skuId=" + ec.selectedProduct.skuId + "&skuIds=" + ec.selectedProduct.skuId + "&activityId=" + accountPars.activityId;
	
	var submitRushbuyJsVersion = ec.util.cookie.get("t_submitCreatOrderUrl") || ec.rushbuy_js_version;
	
	var submitActivity = ec.util.cookie.get("t_submitActivity") || ec.url.activity;
	
	var submitChooseComponent = ec.util.cookie.get("t_submitChooseComponent") || ec.url.chooseComponent;
	
    ec.ajax({
        url: submitCreatOrderUrl,
        type: "POST",
        data: accountPars,
        dataType: "json",
        success: function(k) {
            if (k && k.success) {
                ...
                ec.util.cookie.set("orderSign-" + accountPars.activityId + "-" + k.uid, k.orderSign, j);
                if (ec.flowType == 2) {
                    ec.reserveTimes(k, submitCoverFlag)
                } else {
                    var f = submitChooseComponent + "?nowTime=" + currDate + skus;
                    if (ec.isRequestFromVmall) {
                        f = f + "&backUrl=" + encodeURIComponent(ec.paramForVmall.backUrl) + (ec.paramForVmall.giftSkus ? "&optionalGiftIds=" + ec.paramForVmall.giftSkus: "") + (ec.paramForVmall.accessoriesSkus ? "&componentIds=" + ec.paramForVmall.accessoriesSkus: "") + (ec.paramForVmall.diyPackSkus ? "&diyPackSkus=" + ec.paramForVmall.diyPackSkus: "")
                    }
                    window.location.href = f + "&rushbuy_js_version=" + submitRushbuyJsVersion + "&backto=" + encodeURIComponent(submitActivity)
                }
            } else {
				setTimeout(function() {
					ec.excuteSubmit();
...
setStopFlag = function(flag){
	...
	ec.util.cookie.set("t_stopFlag", flag, j);
	if(flag === '0'){
		ec.excuteRush();
	}
}
